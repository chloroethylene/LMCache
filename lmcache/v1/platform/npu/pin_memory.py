...
issues a genuinely async D2H/H2D, removing the per-chunk host stall in the MP
gather/scatter path.
...
class NpuPinMemoryBackend(PinMemoryBackend):
    ...
    def __init__(self) -> None:
        ...
        # a worker thread has one (torch_npu creates it on first device op) but
        # a cache-server thread may not, so each pinning thread must ensure its
        # own.
    ...
    def unpin_memory(self, ptr: int) -> bool:
        ...
        if self._lib is None:
            return False
        if ptr not in self._registered_bases:
            return True
        base = self._registered_bases.pop(ptr)
        ret = self._lib.aclrtHostUnregister(ctypes.c_void_p(base))
        if ret != _ACL_SUCCESS:
            logger.warning(
                "aclrtHostUnregister failed: ptr=%#x base=%#x ret=%d",
                ptr,
                base,
                ret,
            )
